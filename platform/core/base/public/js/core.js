...
function () {
  function Fast() {
    ...
    Fast.initResources();
    Fast.handleCounterUp();
    Fast.initMediaIntegrate();

    if (FastVariables && FastVariables.authorized === '0') {
  ...
  _createClass(Fast, [{
    ...
    value: function countCharacter() {
      $.fn.charCounter = function (max, settings) {
        ...
        settings = $.extend({
          ...
          format: '(%1 ' + FastVariables.languages.system.character_remain + ')',
          pulse: true,
          delay: 0
        }, settings);
        var p, timeout;
    ...
    value: function handlePortletTools() {
      ...
      $('body').on('click', '.portlet > .portlet-title .fullscreen', function (event) {
        ...
        if (portlet.hasClass('portlet-fullscreen')) {
          ...
          var height = Fast.getViewPort().height - portlet.children('.portlet-title').outerHeight() - parseInt(portlet.children('.portlet-body').css('padding-top')) - parseInt(portlet.children('.portlet-body').css('padding-bottom'));
    ...
    value: function showNotice(messageType, message) {
      ...
      switch (messageType) {
        case 'error':
          messageHeader = FastVariables.languages.notices_msg.error;
          break;

        case 'success':
          messageHeader = FastVariables.languages.notices_msg.success;
          break;
      }
    ...
    value: function handleError(data) {
      if (typeof data.errors !== 'undefined' && !_.isArray(data.errors)) {
        Fast.handleValidationError(data.errors);
      } else {
        if (typeof data.responseJSON !== 'undefined') {
          if (typeof data.responseJSON.errors !== 'undefined') {
            if (data.status === 422) {
              Fast.handleValidationError(data.responseJSON.errors);
            }
          } else if (typeof data.responseJSON.message !== 'undefined') {
            Fast.showError(data.responseJSON.message);
          } else {
          ...
          Fast.showError(data.statusText);
        }
      }
    }
  }, {
    ...
    value: function initResources() {
      ...
      Fast.initDatePicker('.datepicker');

      if (jQuery().mCustomScrollbar) {
        Fast.callScroll($('.list-item-checkbox'));
      }
    ...
    value: function initCodeEditor(id) {
      ...
      $('.handle-tool-drag').mousedown(function (event) {
        ...
        $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Fast.onDragTool);
        $(window).on('mouseup', Fast.onReleaseTool);
      });
    }
  }, {
    key: "onDragTool",
    ...
    value: function onReleaseTool() {
      $('body').off('mousemove', Fast.onDragTool);
      $(window).off('mouseup', Fast.onReleaseTool);
    }
  }]);
...
module.exports = __webpack_require__(/*! E:\laragon\www\fast\platform\core\base\resources\assets\js\core.js */"./platform/core/base/resources/assets/js/core.js");
